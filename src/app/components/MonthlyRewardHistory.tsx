    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      ...
      {rewards.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 dark:text-gray-400">暂无奖励记录</p>
        </div>
      ) : (
        <>
                ...
                {rewards.map((reward) => (
                  <tr key={reward.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                      {reward.rewardYear}年{reward.rewardMonth}月
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
                        {REWARD_LEVEL_NAMES[reward.rewardLevel as keyof typeof REWARD_LEVEL_NAMES] || reward.rewardLevel}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                      <div className="space-y-1">
                        <div className="flex space-x-4 text-xs">
                          <span>传播: {reward.promotionScore}</span>
                          <span>短篇: {reward.shortScore}</span>
                          <span>长篇: {reward.longScore}</span>
                          <span>社区: {reward.communityScore}</span>
                        </div>
                      </div>
                    </td>
